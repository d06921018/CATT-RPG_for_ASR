...
def process_line(line, data_folder, language, accented_letters):
    ...
    if language in ["nan-tw"]:
        hanzi, tailo = split_hanzi_tailo(words)
        # breakpoint()
        words = hanzi
        # words = tailo
    ...
    # if language in ["ja", "zh-CN", "zh-TW", "nan-tw"]:
    if language in ["ja", "zh-CN", "zh-TW"]:
        if len(chars) < 3:
            return None
    elif language in ["nan-tw"]:
        if len(chars) < 3:
            return None
...
def create_csv(
    ...
    with open(csv_file_tmp, mode="w", encoding="utf-8") as csv_f:
        ...
        #     process_line(row, data_folder=data_folder, language=language, accented_letters=accented_letters)
        #     breakpoint()
