...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        tokens = torch.stack([self.dsu_map[wav_id][0] for wav_id in wav_ids]).to(self.device)
        ...
        feats = self.modules.reprogramming_layer(feats)

        y = self.modules.enc(inputs_embeds=feats)
    ...
    def load_dsu_map(self):
        self.dsu_map = torch.load(self.hparams.ssl_dsu)
...
if __name__ == "__main__":
    ...
    _, _, test_datasets, label_encoder = dataio_prepare(
        hparams
    )
